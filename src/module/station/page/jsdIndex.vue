<template>
	<div class="oil_invite">
		...
		<div class="btn_wrap ptb40">
			...
			<a href="javascript:void(0);" @click="freeStation" class="station_btn">{{freeBtn}}</a>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	computed: {
		...
		btnWord () {
			...
			} else if (this.userIsLogin&&!this.userLevelCode) {
				return '立即成为' + this.stationName
			} else {
		...
		//免费成为站长按钮名称
		freeBtn () {
			if (this.userLevel < this.partnerLevel) {
				return '立即成为' + this.stationName
			} else {
				return '立即下载同城约单APP'
			}
	...
	methods: {
		...
		getPartnerInfo () {
			fetchPartnerconfList().then(res => {
				if (res.status) {
					...
					this.topPrice = this.partnerInfoList[res.data.list.length-1].price;
					this.topStationName = this.partnerInfoList[res.data.list.length-1].name;
					this.tab = res.data.list.length;
					fetchProfileDetail('partner_send').then(res => {
						if (res.status) {
							this.stationCode = res.data.sysProfileVo.value;
							this.partnerInfoList = this.partnerInfoList.filter((v) => v.code == this.stationCode);
							this.partnerLevel = this.partnerInfoList[0].level;
							this.stationName = this.partnerInfoList[0].name;
							this.level1Rate = this.partnerInfoList[0].invitePartnerBrokerageList[0].value;
							this.level2Rate = this.partnerInfoList[0].invitePartnerBrokerageList[1].value;
							this.level1bro = this.partnerInfoList[0].tradeBrokerageRateList[1].value;
							this.level2bro = this.partnerInfoList[0].tradeBrokerageRateList[1].value;
							if (this.partnerInfoList.length) {
		...
		// 	})
		// },
		loginHref (refresh) {//解决  ios登录后刷新问题
			if(refresh){
				this.getUserLogin();//授权
			}
        },
		//立即成为站长
		goStation () {
			//未登录跳转jsd登录
			if (!this.userIsLogin) {
				if (this.$store.state.platform == 'wx') { //微信环境跳转jsd登录页面
					if (window.location.host.indexOf("ddtkj.cn")>-1) {
						window.location.href = 'http://j5wx.ddtkj.cn/#!/main/login?backUrl='+ window.location.href;
					} else {
						window.location.href = 'http://m.jinshangdai.com/#!/main/login?backUrl='+ window.location.href
					}
				} else { //app调用app登录方法
					if (isAndroid) {
						window.android.toLogin();
					} else {
						window.location.href='jsd://JSDWeb/login?callback=this.loginHref';
					}
				}
			} else if (this.userIsLogin&&!this.userLevelCode) { //登录且不是站长
				fetchPartnerOrderCreate(this.partnerLevelCode,true).then(res => {
					if(res.data.orderNo){
						this.$router.push('/jsdOrderConfirm?orderNo='+res.data.orderNo)
					}
				})
			} else { //登录且是站长
		...
		getUserLogin () {
			 fetchIslogin().then(res => {
			 	if (res.data.login) {
					this.userIsLogin = res.data.login;
					if(localStorage.isCas){
						localStorage.removeItem("isCas")
					}
			 		if (this.userIsLogin) {
			 			this.getUserType(); //获取用户等级
			 		}
			 	}else{
			 		if(getAppFlag('islogin')||this.$store.state.platform=='wx'){//app 登陆 ，wx 页面未登陆，需要执行授权
			 			let url=encodeURIComponent(location.href);
						fetchAppLogin(url,url).then(result=>{
							if(result.status){//已登录
								...
								localStorage.setItem('isCas', 1) //缓存登录状态
								window.location.href=result.casUrl;
							}
						})
			 		}
				}
		...
		freeStation () {
			if (this.userLevel <= this.partnerLevel) {
				freeStation(true).then(res=>{
					if (res.status) {
						this.$router.push('/jsdPaySuccess');
					} else {
						appDownLoad();
					}
				})
