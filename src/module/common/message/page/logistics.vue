...
<div class="clearfix">
	...
	<div class="z_container bg_gray" v-if="list.length">
		<mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :autoFill="false" ref="loadmore">
		    <logistics-item :list="list"></logistics-item>
		</mt-loadmore>
	</div>
	<div v-else>
		<nodata></nodata>
...
import logisticsItem from '../components/itemLogistics';//物流列表组件
import {fetchMessageList} from '../service/getData';    //佣金消息接口
export default {
	data() {
		return {
			title: '物流消息',//头部title
			code:'goodsSend',//物流消息code（接口参数）
			list: [],        //物流消息列表
			page:1,          //当前页数
			size:10,         //页面条数
