<template>
	<div class="bg router_page">
        ...
        <div class="z_bg">
			<div class="z_title">
				<a class="z_back_icon pull_left" href="javascript:history.go(-1)">
		...
		<div class="z_personal_data">
			...
			<p class="z_address">{{memberInfo.address}}<span><i class="iconfont icon-dizhi f26 c_666"></i>{{memberInfo.dist/1000|number(2)}}km</span></p>
		</div>
		<!-- E 头部个人信息 -->
		<!-- S 个人动态 -->
		<div class="z_personal_dynamic">
			<router-link :to="'/dynamicList?mid=' + userInfo.id">
		...
		<div class="z_personal_info">
			...
			<!--<router-link :to="'/goldGrade?mid=' + userInfo.id">
				<span class="c_666 f28">金币等级</span>
				<i class="pull_right iconfont icon-arrow-right c_666"></i>
				<span class="c_333 pull_right mr20">{{memberInfo.rankIntegral}}</span>
			</router-link>-->
			<a href="javascript:;">
				<span class="c_666 f28">已获得赞</span>
				<span class="c_333 f28 pull_right pr30">{{memberInfo.praiseTotal}}次</span>
				<i class="pull_right iconfont icon-dianzan pr20 zan_color"></i>
			</a>
			<div class="z_visitor">
		...
		<div class="z_skill_info" v-if="memberSkillList.length">
            <div class="z_info_box">
				...
				<!-- <a href="javascript:;" v-else>
                    <span class="c_666 f28">未发布视频介绍</span>
                    <span class="pull_right go_publish" @click="publishVideo">去发布</span>
                </a> -->
            </div>
            <div class="z_info_box" v-if="dataSkillDetail.skillImg">
                <h2 class="f28 c_666">技能相册</h2>
                <ul class="clearfix">
            ...
            <!--<div class="z_info_box">
                <a href="javascript:;" class="clearfix">
                    ...
                    <p class="pull_right c_666 f28">{{dataSkillDetail.integralGrade}}</p>
                </a>
            </div>-->
		...
		<!-- E 用户全部评价 -->
		<!-- S 底部按钮 -->
		<div class="l_item_handler plr30 ptb22 f26">
			<router-link class="item_btn" to="/skillType"><i class="iconfont icon-qian2 mr10"></i>去赚钱</router-link>
			<a class="item_btn" href="javascript:void(0);" @click="share"><i class="iconfont icon-fenxiang2 mr10"></i>拉人气</a>
		</div>
		<!-- E 底部按钮 -->
	</div>
</template>
...
import comIcon from 'src/components/com-icon'; //认证icon
...
export default {
	data() {
		return {
			userInfo: {
				id: this.$route.query.mid || getInfo('id') //用户id
			},
			memberInfo: {}, //个人主页用户信息
	...
	methods: {
		...
		//获取个人主页用户信息
		getMemberInfo () {
			memberInfo(this.userInfo.id).then(res => {
				if (res.status) {
		...
		//获取最近访客列表
		getVisitList () {
			visitMemberList(this.userInfo.id).then(res => {
				if (res.status) {
		...
		//获取评价列表
		getEvaluationList () {
			evaluationList(this.userInfo.id,this.dataSkillId).then(res => {
				if (res.status) {
		...
		//个人主页技能列表
		getSkillList () {
			fetchSkillList(this.userInfo.id).then(res => {
				if (res.status) {
					if (!res.data.list.length) {
						return
					}
					this.memberSkillList = res.data.list || [];
					if (this.memberSkillList.length) {
						this.$nextTick(function(){
		...
		//个人动态前三张图片
		getMemberDynamic () {
			memberDynamic(this.userInfo.id).then(res => {
				if (res.status) {
		...
		skillClick (skillId,index) {
			...
			//获取技能id对应的评价列表
			evaluationList(this.userInfo.id,skillId).then(res => {
				if (res.status) {
					this.evaluationList = res.data.list||[];
		...
		//跳转发布视频页面
		publishVideo () {
			this.$router.push('/videoIntroduce?skillId=' + this.dataSkillId);
		}
	}
}
</script>
